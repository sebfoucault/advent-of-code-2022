...
playMove (count, from, to) stacks
    | count == 0    = stacks
    | otherwise     = playMove (count-1, from, to)  (map (\(idx, stack) -> applyMove idx stack (from-1) (to-1) stacks) (zip [0..] stacks))

applyMove :: Int -> [Char] -> Int -> Int -> [[Char]] -> [Char]
applyMove idx stack from to stacks 
    | idx /= to && idx /= from          = stack
    | idx == to                         = head (stacks!!from) : stack
    | idx == from                       = tail stack
